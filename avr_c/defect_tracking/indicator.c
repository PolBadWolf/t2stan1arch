...
// кнопки
  if ( scan_ready()!=0 )  cur_key=read_key();
  else                    cur_key=255;
...
//=========================================
  ...
  if ( n_screen==3 && xpen==0 )
  {
    ...
    Show_m2(0,"сопровождение");
    {
      ...
      {
        if ( ind_def12[j_ind] ==0 )  Show_m2(16+i_ind,"_");
        else                         Show_m2(16+i_ind,"i");
        j_ind--;
        i_ind++;
      }
    }
    // колесо
    if ( d_koleso_tr==0 ) Show_m2(28," ");
    else                  Show_m2(28,"К");
    // рез
    if ( rez_tr==0 )  Show_m2(29,"_");
    else              Show_m2(29,"Р");
    // собраный дефект перед резом
    if ( ind_d_korez==0 )  Show_m2(30,"x");
    else                   Show_m2(30,"D");
    //-----
    if ( cur_key==1 )
    {
  ...
  if ( n_screen==4 && xpen==0 )
  {
    ...
    xpen=1;
    // колесо
    if ( d_koleso_tr==0 ) Show_m2(0,"_");
    else                  Show_m2(0,"К");
    // рез
    if ( rez_tr==0 )  Show_m2(1,"_");
    else              Show_m2(1,"Р");
    // собраный дефект перед резом
    if ( ind_d_korez==0 )  Show_m2(16,"x");
    else               Show_m2(16,"D");
    // сопровождение трубы на станке
    // 1
    ShowDigitZ(3,3,rez_ms_ind[1]);
    if ( rez_ms_def[1]==0 ) Show_m2(6," ");
    else                    Show_m2(6,"D");
    // 2
    ShowDigitZ(8,3,rez_ms_ind[2]);
    if ( rez_ms_def[2]==0 ) Show_m2(11," ");
    else                    Show_m2(11,"D");
    // 3
    ShowDigitZ(19,3,rez_ms_ind[3]);
    if ( rez_ms_def[3]==0 ) Show_m2(22," ");
    else                    Show_m2(22,"D");
    // 4
    ShowDigitZ(24,3,rez_ms_ind[4]);
    if ( rez_ms_def[4]==0 ) Show_m2(27," ");
    else                    Show_m2(27,"D");
    // выход после станка
    if ( def_rez_out==0 ) Show_m2(29,"x");
    else                  Show_m2(29,"D");
    // длина трубы
    //
  ...
  if ( n_screen==5 && xpen==0 )
  {
    ...
    // выход после станка
    if ( def_rez_out==0 ) Show_m2(16,"x");
    else                  Show_m2(16,"D");
    //
    // датчик 1
    if ( trub_tr_1==0 ) Show_m2(3,"t");
    else                Show_m2(3,"T");
    if ( dt_12==0 )     Show_m2(4," ");
    else                Show_m2(4,"=");
    // датчик 2
    if ( trub_tr_2==0 ) Show_m2(5,"t");
    else                Show_m2(5,"T");
    if ( dt_23==0 )     Show_m2(6," ");
    else                Show_m2(6,"=");
    // датчик 3
    if ( trub_tr_3==0 ) Show_m2(7,"t");
    else                Show_m2(7,"T");
    if ( dt_34==0 )     Show_m2(8," ");
    else                Show_m2(8,"=");
    // датчик 4
    if ( trub_tr_4==0 ) Show_m2(9,"t");
    else                Show_m2(9,"T");
    if ( dt_45==0 )     Show_m2(10," ");
    else                Show_m2(10,"=");
    // датчик 5
    if ( trub_tr_5==0 ) Show_m2(11,"t");
    else                Show_m2(11,"T");

    //
    if ( defect1==0 ) Show_m2(19,"x");
    else              Show_m2(19,"D");
    if ( defect2==0 ) Show_m2(21,"x");
    else              Show_m2(21,"D");
    if ( defect3==0 ) Show_m2(23,"x");
    else              Show_m2(23,"D");
    if ( defect4==0 ) Show_m2(25,"x");
    else              Show_m2(25,"D");
    if ( defect5==0 ) Show_m2(27,"x");
    else              Show_m2(27,"D");
    //----------------------------------
    // индикация упора
    if ( upor_flag==0 )
...
// Запрос "настрока оборудования"
  ...
  if ( n_screen==12 && xpen==0 )
  {
    ...
    if ( cur_key==2 )
    {
        if ( psw_temp>0 ) psw_temp--;
        else psw_temp=9;
        timeout_key=10000;
    }
    if ( cur_key==3 )
    {
        if ( psw_temp<9 ) psw_temp++;
        else psw_temp=0;
        timeout_key=10000;
    }
    if ( cur_key==4 )
...
// Выбор устройства для коррекции расстояния :
  ...
  if ( n_screen==21 && xpen==0 )
  {
      ...
      if ( dev_n==5 )   Show_m2( 16,"стыколова РС1   ");
      if ( dev_n==6 )   dev_n++;
      if ( dev_n==7 )   dev_n++;
      if ( dev_n==8 )   dev_n++;
      if ( dev_n==9 )   dev_n++;
